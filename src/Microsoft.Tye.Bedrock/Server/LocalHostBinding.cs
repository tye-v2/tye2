...
public class LocalHostBinding : ServerBinding
{
    ...
    public LocalHostBinding(int port, ConnectionDelegate application,
    ...
    public override async IAsyncEnumerable<IConnectionListener> BindAsync(
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            ipv6Listener =
                await ConnectionListenerFactory.BindAsync(new IPEndPoint(IPAddress.IPv6Loopback, Port),
                    cancellationToken);
        }
        ...
        {
            ipv4Listener =
                await ConnectionListenerFactory.BindAsync(new IPEndPoint(IPAddress.Loopback, Port), cancellationToken);
        }
        catch (Exception ex) when (!(ex is IOException))
        {
            exceptions.Add(ex);

...
{
    private List<(EndPoint EndPoint, int Port, Action<IConnectionBuilder> Application)> _bindings =
        new List<(EndPoint, int, Action<IConnectionBuilder>)>();
    ...
    internal void Apply(ServerBuilder builder)
    {
        ...
        foreach (var binding in _bindings)
        {
            if (binding.EndPoint == null)
            {
                ...
                builder.Bindings.Add(new LocalHostBinding(binding.Port, connectionBuilder.Build(),
